...
<div class="max-w-6xl mx-auto">
    ...
    <div class="text-center mb-12">
        <h1 class="text-4xl font-bold text-gray-900 mb-4">Boost Your Listing with Premium Placement</h1>
    ...
    @if($activePremiumPackages->count() > 0)
        <div class="bg-gradient-to-r from-purple-50 to-blue-50 border-2 border-purple-300 rounded-xl p-6 mb-8">
            <div class="flex items-start">
                ...
                <div class="flex-1">
                    <h3 class="text-lg font-bold text-purple-900 mb-3">🎉 Your Listing Has Active Premium Packages!</h3>
                    <div class="space-y-2">
                        @foreach($activePremiumPackages as $package)
                            <div class="flex items-center justify-between bg-white rounded-lg px-4 py-2 border {{ $package->package_type === 'hero' ? 'border-blue-200' : 'border-purple-200' }}">
                                <div class="flex items-center">
                                    @if($package->package_type === 'hero')
                                        <span class="text-blue-600 font-bold mr-2">🎯</span>
                                        <span class="font-semibold text-blue-700">Hero Banner</span>
                                    @else
                                        <span class="text-purple-600 font-bold mr-2">⭐</span>
                                        <span class="font-semibold text-purple-700">Featured Listing</span>
                                    @endif
                                </div>
                                <span class="text-sm text-gray-600">Until {{ $package->expires_at->format('M d, Y') }}</span>
                            </div>
                        @endforeach
                    </div>
                    <p class="text-sm text-gray-600 mt-3">You can purchase additional packages or extend existing ones below.</p>
                </div>
            </div>
        </div>
    @endif
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        @foreach($packages as $type => $package)
            <div class="bg-white rounded-2xl shadow-xl border-2 {{ $type === 'hero' ? 'border-blue-500' : 'border-purple-500' }} overflow-hidden">
                <div class="bg-gradient-to-r {{ $type === 'hero' ? 'from-blue-600 to-cyan-600' : 'from-purple-600 to-blue-600' }} text-white text-center py-3 font-bold text-lg">
                    {{ $type === 'hero' ? '🎯 HERO BANNER' : '⭐ FEATURED LISTING' }}
                </div>

                <div class="p-8">
                    {{-- Package Icon --}}
                    <div class="w-20 h-20 mx-auto mb-6 rounded-full bg-gradient-to-br {{ $type === 'hero' ? 'from-blue-100 to-cyan-100' : 'from-purple-100 to-blue-100' }} flex items-center justify-center">
                        @if($type === 'hero')
                    ...
                    {{-- Package Name --}}
                    <h2 class="text-2xl font-bold text-center {{ $type === 'hero' ? 'text-blue-900' : 'text-purple-900' }} mb-2">{{ $package['name'] }}</h2>
                    
                    @if(isset($package['description']))
                        <p class="text-center text-gray-600 text-sm mb-6">{{ $package['description'] }}</p>
                    @endif

                    {{-- Price --}}
                    <div class="text-center mb-6">
                        <span class="text-4xl font-bold text-gray-900">{{ rupiah($package['price']) }}</span>
                        <p class="text-gray-600 text-sm mt-2">{{ $package['duration_days'] }} days</p>
                    </div>

                    {{-- Features --}}
                    <div class="bg-gray-50 rounded-xl p-6 mb-6">
                        <h3 class="font-bold text-gray-900 mb-3 text-sm">What You'll Get:</h3>
                        <ul class="space-y-2">
                    ...
                    <form action="{{ route('premium.createPayment', $item->id) }}" method="POST">
                        ...
                        <button type="submit" class="w-full py-3 px-6 bg-gradient-to-r {{ $type === 'hero' ? 'from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700' : 'from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700' }} text-white font-bold rounded-xl transition-all shadow-lg hover:shadow-xl transform hover:scale-105 cursor-pointer">
