    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @if ($transactions->isEmpty())
            ...
            <div class="text-center py-12 bg-white rounded-lg shadow">
                ...
                <p class="mt-1 text-sm text-gray-500">You haven't participated in any transactions yet.</p>
                <div class="mt-6">
                    <a href="{{ route('items') }}" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                                ...
                                                <div class="text-sm text-gray-600 space-y-1">
                                                    ...
                                                    <div class="flex items-center gap-4 mt-2">
                                                        <span class="flex items-center">
                                                            ...
                                                            @else
                                                                <svg class="h-5 w-5 text-gray-300 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                    ...
                                    <div class="ml-4 flex flex-col items-end gap-2">
                                        @if ($transaction->status === 'pending' && $transaction->seller_id === auth()->id())
                                            <a href="{{ route('transaction.links', $transaction->id) }}" 
                                               class="inline-flex items-center px-3 py-1.5 border border-blue-600 text-xs font-medium rounded-md text-blue-600 bg-white hover:bg-blue-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                        ...
                                        @if ($transaction->status === 'completed')
                                            <a href="{{ route('transaction.report', $transaction->id) }}" 
                                               class="inline-flex items-center px-3 py-1.5 border border-emerald-600 text-xs font-medium rounded-md text-emerald-600 bg-white hover:bg-emerald-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500">
                                                <svg class="h-4 w-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                                </svg>
                                                View Report
                                        ...
                                        @if ($transaction->status === 'pending' && $transaction->seller_id === auth()->id())
                                            <form method="POST" action="{{ route('transaction.cancel', $transaction->id) }}" 
                                                ...
                                                @csrf
                                                <button type="submit" 
                                                        class="inline-flex items-center px-3 py-1.5 border border-red-300 text-xs font-medium rounded-md text-red-600 bg-white hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 cursor-pointer">
                                                    Cancel
                                                </button>
                                            </form>
                                        @endif
