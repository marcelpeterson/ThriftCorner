...
<div class="max-w-4xl mx-auto px-4 py-2 max-md:mt-2">
    <div class="mb-6 sm:mb-8">
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Edit Listing</h1>
        <p class="mt-2 text-xs sm:text-sm text-gray-600">Update the details of your item listing on ThriftCorner</p>
    </div>
    ...
    <form action="{{ route('items.edit.submit', $item->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6 sm:space-y-8">
        @csrf
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
            <div class="flex items-center mb-4">
                ...
                <h2 class="text-base sm:text-xl font-bold text-gray-900">Manage Images</h2>
            </div>
            <p class="text-xs sm:text-sm text-gray-600 mb-4">Upload new images or manage existing ones. The first image will be the cover photo.</p>
                ...
                <div class="flex items-center justify-center w-full">
                    <button type="button" onclick="showPhotoGuidelinesModal()" class="flex flex-col items-center justify-center w-full h-48 sm:h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 transition-colors">
                        <div class="flex flex-col items-center justify-center pt-4 sm:pt-5 pb-4 sm:pb-6 px-4">
                            ...
                            <p class="text-xs text-gray-500 mt-1">Up to 5 images total</p>
                        </div>
                    </button>
                    <input id="images" name="images[]" type="file" class="hidden" accept="image/*" multiple onchange="previewImages(this)"/>
                </div>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select id="condition" name="condition" required
                        ...
                        <option value="Lightly used" {{ old('condition', $item->condition) == 'Lightly used' ? 'selected' : '' }}>Lightly used</option>
                        <option value="Well used" {{ old('condition', $item->condition) == 'Well used' ? 'selected' : '' }}>Well used</option>
                        <option value="Heavily used" {{ old('condition', $item->condition) == 'Heavily used' ? 'selected' : '' }}>Heavily used</option>
                    </select>
                    @error('condition')
                        <p class="mt-1 text-xs sm:text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" id="price" name="price" value="{{ old('price', $item->price) }}" required min="0" max="99999999" step="1"
                               placeholder="0"
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4 sm:rows-6" maxlength="2000"
                              ...
                              class="w-full px-3 sm:px-4 py-2 text-xs sm:text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent @error('description') border-red-500 @enderror">{{ old('description', $item->description) }}</textarea>
                    <p class="mt-1 text-xs text-gray-500">
                        <span id="char-count">{{ strlen($item->description ?? '') }}</span> / 2000 characters
                    </p>
                    @error('description')
                        <p class="mt-1 text-xs sm:text-sm text-red-600">{{ $message }}</p>
                    @enderror
...
<script>
    ...
    function previewImages(input) {
        ...
        if (existingImageCount + files.length > 5) {
            alert('You can only have a maximum of 5 images total (existing + new).');
            input.value = '';
            previewContainer.classList.add('hidden');
            return;
        }
